...
exports.headergen = function (MID, httpmethod, URI, timestamp, nonce, payload, APIkey) {
   ...
   // var md5 = require("crypto-js)
    ...
    function hm(string)
{
    var apibase64 = Buffer.from(apikey, 'base64');    
    var rawbytes = Buffer.from(apibase64, 'utf8');
    
    return crypto.createHmac('sha256', rawbytes).update(rawbytes).digest('base64');
}
    ...
    try {
       ...
       // var rawbytes = Buffer(string, 'utf8');
    
        var rs = crypto.createHmac('sha256', apibase64).update("CFF7D059-5EFC-49B2-BD18-B937261943B5POSThttps://previewapi.paidbycoins.com/v1/cli/createpayment20180725 10:00:0012345gpLRVWfYAJ9AwhRFFlKCaA==").digest('base64');
